            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="vibrant-card vibrant-card-indigo p-8 animate-fade-up" style="animation-delay: 0.1s">
                    <div class="flex items-center">
                        <div class="w-16 h-16 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-100 me-6" style="background: var(--primary-gradient)">
                ...
                <div class="vibrant-card vibrant-card-success p-8 animate-fade-up" style="animation-delay: 0.2s">
                    <div class="flex items-center">
                        <div class="w-16 h-16 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-100 me-6" style="background: var(--success-gradient)">
                ...
                <div class="vibrant-card vibrant-card-danger p-8 animate-fade-up" style="animation-delay: 0.3s">
                    <div class="flex items-center">
                        <div class="w-16 h-16 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-rose-100 me-6" style="background: var(--danger-gradient)">
                        ...
                        <tbody>
                            @forelse($customer->invoices as $invoice)
                                <tr class="hover:bg-slate-50/50 transition border-b border-slate-50 last:border-0">
                                    <td class="py-5 font-bold text-slate-600">#{{ $invoice->invoice_no }}</td>
                                    <td class="py-5 text-slate-400 font-medium">{{ Carbon\Carbon::parse($invoice->invoice_date)->format('M d, Y') }}</td>
                                    <td class="py-5 font-black text-slate-800">Rs. {{ number_format($invoice->amount, 2) }}</td>
                                    <td class="py-5 text-emerald-600 font-bold">Rs. {{ number_format($invoice->received_amount, 2) }}</td>
                                    <td class="py-5 text-rose-600 font-bold">Rs. {{ number_format($invoice->due_amount, 2) }}</td>
